...
def _get_decision_votes(case: dict, voted_in_favor = True):
    return [vote for decision in case.get(DECISION) for vote in decision.get(VOTE) if vote.get(TYPE) == (FOR if voted_in_favor else AGAINST)]
...
def _get_motion_text(motion) -> str:
    ...
    for split_index, par in enumerate(pars):
        if "MOTIE VAN" in par:
            # we found the split index.
            break
    motion_text = "\n".join(pars[split_index + 1 :])
    return motion_text
